...
.top5-badge {{
  ...
  align-items: center;
  gap: 6px;
  background: linear-gradient(135deg, rgba(15,130,140,0.25), rgba(6,80,132,0.20));
...
# ══════════════════════════════════════════════════════════════════════════════
# ── SIDEBAR — TOP 5 INPUTS ONLY ───────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
#
# Feature importance ranking (from RandomForest on full 41-feature model):
#   #1  TotalCharges              0.16882  ← auto-computed: tenure × MonthlyCharges
#   #2  tenure                    0.14424  ← slider
#   #3  MonthlyCharges            0.14381  ← slider
#   #4  Contract_Month-to-month   0.05539  ← selectbox (Contract)
#   #5  OnlineSecurity_0          0.03287  ← selectbox (OnlineSecurity)
#
# TotalCharges is the #1 feature but is a mathematical product of tenure × monthly
# (Pearson r = 0.9996), so it is auto-computed and requires no extra user input.
# The remaining 36 low-importance features are silently set to their neutral defaults.
# ──────────────────────────────────────────────────────────────────────────────
...
st.sidebar.markdown(
    f"<p style='font-size:12px;color:rgba(212,234,233,0.65);margin:-8px 0 12px;'>"
    f"Only the highest-impact signals are shown.<br>"
    f"All other model inputs use safe defaults.</p>",
    unsafe_allow_html=True
)

st.sidebar.markdown("---")

# ── Feature #2 & #3: Tenure + MonthlyCharges ──────────────────────────────────
st.sidebar.markdown(
...
# Feature #1 is auto-computed — show it as an info note
total_charges_est = tenure * monthly
st.sidebar.markdown(
    f"<div style='background:rgba(15,130,140,0.12);border:1px solid rgba(120,185,181,0.25);"
    f"border-radius:8px;padding:10px 14px;margin:8px 0 16px;font-size:12px;"
    f"color:rgba(212,234,233,0.75);'>"
    f"<strong style='color:{TEAL_LIGHT};'>① TotalCharges</strong> · #1 feature<br>"
    f"Auto-computed: {tenure} × ₹{monthly} = <strong style='color:{TEAL_LIGHT};'>"
...
st.sidebar.markdown("---")
st.sidebar.markdown(
    f"<p style='font-size:11px;color:rgba(212,234,233,0.45);text-align:center;'>"
    f"36 remaining model features<br>set to neutral baseline defaults</p>",
    unsafe_allow_html=True
)
...
# ══════════════════════════════════════════════════════════════════════════════
# All 41 columns from columns.pkl are populated.
# The 5 user-provided features are set from sidebar inputs.
# The remaining 36 are set to their statistically neutral / modal defaults.
# ──────────────────────────────────────────────────────────────────────────────

def build_input_row(tenure, monthly, contract, online_security):
    """
    Returns a single-row DataFrame aligned to the model's expected columns.
    Top-5 features come from user input; all others use safe defaults.
    """
    # --- start with all zeros ---
    row = {col: 0 for col in columns}

    # ── Top-5 features (user-supplied) ───────────────────────────────────────
    # #1 TotalCharges (auto-derived)
    ...
    # #5 OnlineSecurity one-hot  (0 = No, 1 = Yes, NIS = No internet service)
    ...
    # ── Neutral / modal defaults for remaining 36 features ───────────────────
    # Demographics — modal values from Telco dataset
    row["SeniorCitizen"]  = 0          # ~84% are non-senior
    row["Partner"]        = 0          # modal = No
    row["Dependents"]     = 0          # modal = No
    row["PhoneService"]   = 1          # ~90% have phone service
    row["PaperlessBilling"] = 1        # modal = Yes

    # Gender (neutral — balanced split, zero out both = baseline)
    row["gender_Female"]  = 0
    row["gender_Male"]    = 0

    # MultipleLines — modal = No
    row["MultipleLines_0"]               = 1
    row["MultipleLines_1"]               = 0
    row["MultipleLines_No phone service"] = 0

    # InternetService — modal = Fiber optic (most common in dataset)
    row["InternetService_0"]           = 0
    row["InternetService_DSL"]         = 0
    row["InternetService_Fiber optic"] = 1

    # OnlineBackup — modal = No
    row["OnlineBackup_0"]                   = 1
    row["OnlineBackup_1"]                   = 0
    row["OnlineBackup_No internet service"] = 0

    # DeviceProtection — modal = No
    row["DeviceProtection_0"]                   = 1
    row["DeviceProtection_1"]                   = 0
    row["DeviceProtection_No internet service"] = 0

    # TechSupport — modal = No
    row["TechSupport_0"]                   = 1
    row["TechSupport_1"]                   = 0
    row["TechSupport_No internet service"] = 0

    # StreamingTV — modal = No
    row["StreamingTV_0"]                   = 1
    row["StreamingTV_1"]                   = 0
    row["StreamingTV_No internet service"] = 0

    # StreamingMovies — modal = No
    row["StreamingMovies_0"]                   = 1
    row["StreamingMovies_1"]                   = 0
    row["StreamingMovies_No internet service"] = 0

    # PaymentMethod — modal = Electronic check
    ...
    # Build DataFrame aligned to model column order
...
with tab1:
    ...
    # Show active top-5 inputs summary
    st.markdown(f"""
    <div class="churn-card" style="padding:18px 24px;margin-bottom:18px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-top:12px;">
        <div><span style="color:{MUTED};font-size:11px;">① TOTAL CHARGES (auto)</span>
             <div style="font-weight:700;color:{TEAL_LIGHT};font-size:16px;">₹{total_charges_est:,.0f}</div></div>
        <div><span style="color:{MUTED};font-size:11px;">② TENURE</span>
             <div style="font-weight:700;color:{TEAL_LIGHT};font-size:16px;">{tenure} mo</div></div>
        <div><span style="color:{MUTED};font-size:11px;">③ MONTHLY CHARGES</span>
             <div style="font-weight:700;color:{TEAL_LIGHT};font-size:16px;">₹{monthly}</div></div>
        <div><span style="color:{MUTED};font-size:11px;">④ CONTRACT</span>
             <div style="font-weight:700;color:{TEAL_LIGHT};font-size:16px;">{contract}</div></div>
        <div><span style="color:{MUTED};font-size:11px;">⑤ ONLINE SECURITY</span>
             <div style="font-weight:700;color:{TEAL_LIGHT};font-size:16px;">{online_security}</div></div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <style>
    /* target the specific predict button by key via DOM order fallback */
    div[data-testid="element-container"]:has(div[data-testid="stButton"] button) .stButton > button {
        /* override only for the main predict CTA – handled by key-based container below */
    }
    </style>
    """, unsafe_allow_html=True)
    if st.button("⚡ PREDICT CHURN RISK", key="predict_main", type="primary", use_container_width=True):
        ...
        try:
            ...
            # Helvetica cannot encode ₹ — use "Rs." as safe ASCII fallback
            ...
            industry_avg  = 26.0   # telecom industry average churn %

            # Churn drivers text
            drivers = []
            if contract == "Month-to-month":
                drivers.append(("Contract Type", "Month-to-month customers are 3x more likely to churn than annual subscribers. No long-term commitment means zero switching cost for the customer."))
            if online_security == "No":
                drivers.append(("No Online Security", "Customers without add-on services show higher churn. Add-ons increase perceived value and raise the cost of switching to a competitor."))
            if tenure <= 12:
                drivers.append(("Low Tenure", f"At {tenure} month(s), this customer is in the highest-risk early churn window (months 1-12). Loyalty has not yet been established."))
            if monthly > 1000:
                drivers.append(("High Monthly Charges", f"At {_R}{monthly:,}/mo, this customer pays above the high-value threshold. Price sensitivity and perceived value mismatch are key churn triggers."))
            if not drivers:
                drivers.append(("Strong Retention Profile", "No major churn risk factors detected. This customer exhibits characteristics associated with long-term retention."))
            ...
            if tenure <= 12:
                actions.append((priority, "Launch Early Loyalty Programme",
                    "Enrol the customer in a months 3-12 loyalty scheme with "
                    "reward points, free data top-ups, or waived fees. "
                    "Early intervention reduces churn by up to 25%."))
                priority += 1
            if online_security == "No":
                actions.append((priority, "Upsell Online Security Add-on",
                    "Offer a 3-month free trial of Online Security. "
                    "Customers with 2+ add-ons churn 40% less. "
                    "Bundle with device protection for higher uptake."))
                priority += 1
            if monthly > 1000:
                actions.append((priority, "Conduct Pricing Review Call",
                    "Assign a dedicated account manager to review the customer's "
                    "plan. Offer a plan optimisation that reduces the bill by "
                    "Rs.100-200/mo while retaining core services."))
                priority += 1
            if not actions:
                actions.append((1, "Proactive Satisfaction Check-in",
                    "Schedule a quarterly satisfaction call. Introduce the "
                    "customer to new features or services to deepen engagement. "
            ...
            class PDF(FPDF):
                ...
                def section_title(self, txt):
                    ...
                    self.cell(0, 8, f"  {txt}", new_x="LMARGIN", new_y="NEXT",
                              fill=True)
            ...
            pdf.multi_cell(0, 6,
                ...
                "feature importances (TotalCharges, Tenure, MonthlyCharges, Contract, "
                "OnlineSecurity):")
            pdf.ln(2)
            ...
            pdf.two_col("Return on Investment:",            f"{roi_ratio:.1f}x  (every {_R}1 spent returns {_R}{roi_ratio:.1f})")
            pdf.ln(4)
    ...
    try:
        ...
        imp_df["Color"]  = imp_df["IsTop5"].apply(
            lambda x: TEAL_LIGHT if x else "rgba(120,185,181,0.35)"
        )
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.caption(f"Highlighted bars ({TEAL_LIGHT} color) = Top-5 features used in the form above.")
    except Exception as e:
...
<div style="text-align:center;color:{MUTED};font-size:13px;padding-bottom:20px;">
  ...
  &nbsp;·&nbsp; <strong style="color:{TEAL_LIGHT};">Developed by Mahreen Begum</strong>
</div>
""", unsafe_allow_html=True)
